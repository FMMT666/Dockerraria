...
if [ $# -eq 0 ]; then
	/usr/games/cowsay "This could be us, but playing with this Terraria 1.4.0.4 server.\
 ...
 'terraria.cfg' file instead) or type 'bash' to get a shell. *burp*"
	...
	if [ "$1" == "config" ]; then
		/1404/Linux/TerrariaServer.bin.x86_64 -config /terraria/terraria.txt
	else
		/1404/Linux/TerrariaServer.bin.x86_64 "$@"
	fi
